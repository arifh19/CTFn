...
def do_register(name, email, password, cas_name=None, full_name=None):
    logger = logging.getLogger('regs')

    with app.app_context():
        team = Teams(name, email.lower(), password)
        if cas_name:
            team.cas_name = cas_name
        if full_name:
        ...
        if utils.can_send_mail() and utils.get_config(
                'verify_emails'):  # Confirming users is enabled and we can send email.
            logger = logging.getLogger('regs')
            ...
            if utils.can_send_mail():  # We want to notify the user that they have registered.
                utils.sendmail(email,
                               "You've successfully registered for {}".format(utils.get_config('ctf_name')))
...
@auth.route('/netsos/secret/chamber/register', methods=['POST', 'GET'])
def register():
    logger = logging.getLogger('regs')
    ...
    if request.method == 'POST':
        ...
        return do_register(name, email, password)
...
def do_login(name, password):
    ...
    if team:
        if team and bcrypt_sha256.verify(password, team.password):
            try:
...
@auth.route('/cas/after-login')
@login_required
def cas_after_login():
    cas_name = CAS().username
    team = Teams.query.filter_by(cas_name=cas_name).first()
    if team:
        return do_login(team.name, app.config['CAS_DEFAULT_PASSWORD'])
    else:
        return redirect(url_for('auth.cas_register'))

@auth.route('/cas/after-logout')
def cas_after_logout():
    return redirect(url_for('views.static_html'))


@auth.route('/cas/register', methods=['GET', 'POST'])
@login_required
def cas_register():
    cas = CAS()
    cas_name = cas.username
    cas_full_name = cas.attributes['cas:nama']
    errors = []
    if request.method == 'POST':
        name = request.form['name']
        email = request.form['email']
        ...
        if len(errors) > 0:
            return render_template('register-cas.html', errors=errors, cas_name=cas_name, cas_full_name=cas_full_name,
                                   name=request.form['name'], email=request.form['email'])

        return do_register(name, email, app.config['CAS_DEFAULT_PASSWORD'], cas_name, cas_full_name)
